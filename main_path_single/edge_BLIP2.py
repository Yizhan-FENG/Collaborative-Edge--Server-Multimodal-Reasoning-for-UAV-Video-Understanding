...
VIDEO_PATH = "/data3/health/wyy/data/7m30s.mp4"
# VIDEO_PATH = "/data3/health/data/无人机第一视角视频.mp4"

DEVICE_ID = "cuda:5"

# 定义结果保存路径
OUTPUT_BASE_DIR = "/data3/health/wyy/BLIP2/final/main_path/edge_output_videos/lunwen" 
RESULTS_JSONL_PATH = os.path.join(OUTPUT_BASE_DIR, "video_analysis_results.jsonl")
KEYFRAMES_SAVE_DIR = os.path.join(OUTPUT_BASE_DIR, "keyframes")

def save_results(record_time, video_duration, keyframe_count, raw_frame_count, analysis_report, frame_descriptions, rep_frames, rep_tstamps, yolo_log, video_path=VIDEO_PATH):
    """
...
def get_representative_frames(features, frames, timestamps, max_frames=50):
    """
    聚类函数
    Args:
        features: 一次抽帧的视觉特征
    ...
    # 计算聚类数量，但不超过MAX_KEYFRAMES
    if len(features) <= MAX_KEYFRAMES:
        K = len(features)  # 如果特征数量小于等于50，就用全部
    else:
        # 如果特征数量大于50，使用50个聚类
        K = MAX_KEYFRAMES
...
def run_once():
    ...
    # 2. 原始抽帧 (Sample)
    # sample_rate=2 表示每秒采2帧，你可以根据需要改
    feats, frames, tstamps, video_duration = extract_features_for_clustering(VIDEO_PATH, blip_model, processor, device, sample_rate=0.5)
    raw_frame_count = len(frames) # 【新增】记录原始抽帧数
    # print("一次抽帧数量", raw_frame_count)

    # 3. 聚类筛选关键帧 (Cluster)
    rep_frames, rep_tstamps = get_representative_frames(feats, frames, tstamps)
    keyframe_count = len(rep_frames) # 【新增】记录关键帧数
    ...
    # 6.视觉前缀和文本embedding
    inputs_embeds = torch.cat([vid_embeds, text_embeds], dim=1) # 形状: (1, num_visual_tokens+text_len, hidden_size)

    combined_attention_mask = torch.cat([vid_mask.to(torch.long), text_inputs.attention_mask], dim=1) # 形状: (1, num_visual_tokens+text_len)
    ...
    with torch.no_grad():
        generated_ids = blip_model.language_model.generate(
            ...
            max_new_tokens=1024,          # 根据报告长度调整
            do_sample=False,             # 设置为True可以进行采样生成，但确定性任务通常用False
    ...
    print("="*50)
    print("程序运行时间:", record_time)
    print("关键帧数量", keyframe_count)
    print("原始视频长度", video_duration)
