...
# 输出目录
OUTPUT_BASE_DIR = "/data3/health/wyy/BLIP2/experiment/Ablation_experiment/only_keyframe/description/qwen3_vl_results"
...
def process_single_video(folder_path, model, processor, device, jsonl_writer=None):
    """
    处理单个视频的核心函数 - 消融实验：只使用关键帧，不使用文本信息
    ...
    # 2. 构建system prompt (消融实验：只使用视觉信息)
    system_prompt = """You are a video captioning model. Your task is to generate a SINGLE, CONCISE English sentence that summarizes the main activity or event visible in the provided keyframe images.
...
def main():
    """主函数：批量处理所有视频（消融实验：只使用关键帧）"""
    print("=" * 60)
    print("Qwen3-VL 多视频批量处理系统 (消融实验：只使用关键帧)")
    print("=" * 60)
    ...
    jsonl_filename = f"video_analysis_results_only_keyframes_{timestamp}.jsonl"
    jsonl_path = os.path.join(OUTPUT_BASE_DIR, jsonl_filename)
    
    print(f"\n📁 创建JSONL输出文件: {jsonl_path}")
    
    # 4. 批量处理视频
    print(f">>> 开始批量处理 {len(video_folders)} 个视频")
    ...
    with open(jsonl_path, 'w', encoding='utf-8') as jsonl_file:
        # 使用tqdm显示进度条
        for i, folder_path in enumerate(tqdm(video_folders, desc="处理进度")):
    ...
    print("批量处理完成！")
    print("=" * 60)
    print(f"总视频数: {len(video_folders)}")
    ...
    summary = {
        ...
        "failed": failed_count,
        "task_type": "video description (ablation: only keyframes)",
        "jsonl_file": jsonl_path,
        "experiment_type": "Ablation: Only Keyframes (No Text Information)",
    ...
    summary_path = os.path.join(OUTPUT_BASE_DIR, f"batch_process_summary_only_keyframes_{timestamp}.json")
    ...
    if failed_count > 0:
        ...
        for result in results:
            if result.get('status') == 'error':
                print(f"  视频 {result.get('video_id', 'unknown')}: {result.get('error', '未知错误')}")
